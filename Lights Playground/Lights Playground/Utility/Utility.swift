...
func calculateControlValueRight(for finger: GloveFinger, value flexValue: Int, controlType: LightControlType) -> Int
{
    let difference = Float(flexValue - GloveConstants.RIGHT_FLEX_MIN[finger]!)
    var highVal: Float = 100.0
    if controlType == .Hue
    {
        highVal = 255
    }
    ...
    let multiplier = highVal / Float(GloveConstants.RIGHT_FLEX_MAX[finger]! - GloveConstants.RIGHT_FLEX_MIN[finger]!)
    var toReturn = difference * multiplier + 1
    if toReturn > highVal
    {
        toReturn = highVal
    }
